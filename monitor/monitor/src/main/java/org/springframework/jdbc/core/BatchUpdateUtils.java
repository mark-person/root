...
import com.ppx.cloud.monitor.context.MonitorContext;
...
public abstract class BatchUpdateUtils {
    ...
    public static int[] executeBatchUpdate(
        ...
        // dengxz
        MonitorContext.getAccessLog().addSqlBeginTime(System.currentTimeMillis());
        long nanoTime = System.nanoTime();
        
        if (batchValues.size() > 0) {
            // 取第一条的参数
            int sqlIndex = MonitorContext.getAccessLog().getSqlList().size();
            MonitorContext.getAccessLog().addSqlArg(sqlIndex, batchValues.get(0));
        }
        ...
        if (r != null) {
            MonitorContext.getAccessLog().addSqlCount(r.length);
        }
        MonitorContext.getAccessLog().addSqlSpendTime((System.nanoTime() - nanoTime) / 1000000);
        return r;
    }
